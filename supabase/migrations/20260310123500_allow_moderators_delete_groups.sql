-- Allow moderators/admins (global) to delete any group (only if groups table exists)
...
-- Update RPC delete_group to allow group admins OR global moderator/admin (only if groups table exists)
DO $$
BEGIN
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'groups') THEN
