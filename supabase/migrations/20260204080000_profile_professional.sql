-- Profile professional and achievements tables (only if profiles table exists)
...
-- Create the update_updated_at_column function outside the DO block
...
DO $$
BEGIN
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'profile_professional') THEN
    ...
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'profile_achievements') THEN
