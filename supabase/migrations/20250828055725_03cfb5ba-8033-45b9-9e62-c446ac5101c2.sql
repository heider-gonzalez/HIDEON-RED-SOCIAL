-- Create post_shares table to track when users share posts (only if table doesn't exist)
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'post_shares') THEN
